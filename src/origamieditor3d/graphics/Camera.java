...
public class Camera {
    ...
    public double[] projection0(double[] point) {
        ...
        double[] basepoint = Geometry.linePlaneIntersection(
                point, camDirection, camPosition, Geometry.scalarMultiple(camDirection, zoom));

        double[] img = {basepoint[0] * xAxis[0] * zoom + basepoint[1] * xAxis[1] * zoom + basepoint[2] * xAxis[2] * zoom,
    ...
    public double[] deprojection(double... xy) {
        ...
        double X = (xy[0] - xShift
                + new Camera(xShift, yShift, zoom).projection0(camPosition)[0]) / zoom;

        double Y = (xy[1] - yShift
                + new Camera(xShift, yShift, zoom).projection0(camPosition)[1]) / zoom;
    ...
    public void rotate(float x, float y) {
        ...
        camDirection = Geometry.normalizeVector(Geometry.rotation(camDirection, Geometry.nullvector, yAxis, sinX, cosX));
        ...
        camDirection = Geometry.normalizeVector(Geometry.rotation(camDirection, Geometry.nullvector, xAxis, sinY, cosY));
        ...
        for (int n : denoms) {
            for (int i = 0; i < origami.getPolygonsSize(); i++) {
                if (origami.isNonDegenerate(i)) {
                    ...
                    double[] last1 = origami.getVertices().get(origami.getPolygons().get(i).get(origami.getPolygons().get(i).size() - 1));
    ...
    public java.util.List<int[]> alignmentPoints2d(Origami origami) {
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (origami.isNonDegenerate(i)) {
                ...
                double[] Upont1 = origami.getVertices2d().get(origami.getPolygons().get(i).get(origami.getPolygons().get(i).size() - 1));
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (isDrawable(i, origami)) {
                ...
                for (int ii = 0; ii < origami.getPolygons().get(i).size(); ii++) {
                    ...
                    edges.addPoint((short) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[0]) + xShift,
                            (short) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[1]) + yShift);
                }
    ...
    public String drawEdges(int x, int y, Origami origami) {
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (isDrawable(i, origami)) {
                ...
                edges += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[0]) + x);
                edges += " ";
                edges += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[1]) + y);
                edges += " m ";

                for (int ii = 1; ii < origami.getPolygons().get(i).size(); ii++) {
                    edges += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[0]) + x);
                    edges += " ";
                    edges += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[1]) + y);
                    edges += " l ";
                }
                edges += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[0]) + x);
                edges += " ";
    ...
    public String drawSelection(int x, int y, double[] ppoint, double[] pnormal, int polygonIndex, Origami origami) {
        ...
        for (int i : kijeloles) {
            ...
            if (isDrawable(i, origami)) {
                ...
                selection += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[0]) + x);
                selection += " ";
                selection += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[1]) + y);
                selection += " m ";

                for (int ii = 1; ii < origami.getPolygons().get(i).size(); ii++) {
                    selection += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[0]) + x);
                    selection += " ";
                    selection += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[1]) + y);
                    selection += " l ";
                }
                selection += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[0]) + x);
                selection += " ";
    ...
    public boolean isDrawable(int polygonIndex, Origami origami, int... ref) {
        ...
        if (origami.getPolygons().get(polygonIndex).size() > 2) {
            ...
            for (int i = 0; i < origami.getPolygons().get(polygonIndex).size(); i++) {
                ...
                int pont0ind = origami.getPolygons().get(polygonIndex).get((i + 1) % origami.getPolygons().get(polygonIndex).size());
                int pont2ind = origami.getPolygons().get(polygonIndex).get((i + 2) % origami.getPolygons().get(polygonIndex).size());
                double area = Geometry.vectorLength(Geometry.crossProduct(Geometry.vectorDiff(origami.getVertices().get(pont1ind), origami.getVertices().get(pont0ind)),
    ...
    public void drawGradient(Graphics canvas, int rgb, Origami origami) {
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (isDrawable(i, origami)) {
                ...
                double[] fnormal = Geometry.crossProduct(Geometry.vectorDiff(origami.getVertices().get(origami.getPolygons().get(i).get(0)),
                ...
                for (int ii = 0; ii < origami.getPolygons().get(i).size(); ii++) {
                    ...
                    path.addPoint((short) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[0]) + xShift,
                            (short) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[1]) + yShift);

                    double sc = Geometry.scalarProduct(origami.getVertices().get(origami.getPolygons().get(i).get(ii)), camDirection);
    ...
    public String drawFaces(int x, int y, Origami origami) {
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (isDrawable(i, origami)) {
                ...
                out += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[0]) + x);
                out += " ";
                out += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[1]) + y);
                out += " m ";

                for (int ii = 1; ii < origami.getPolygons().get(i).size(); ii++) {
                    out += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[0]) + x);
                    out += " ";
                    out += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(ii)))[1]) + y);
                    out += " l ";
                }
                out += Integer.toString((int) (projection(origami.getVertices().get(origami.getPolygons().get(i).get(0)))[0]) + x);
                out += " ";
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (isDrawable(i, origami)) {
                ...
                for (int ii = 0; ii < origami.getPolygons().get(i).size(); ii++) {
                    ...
                    ut.addPoint((short) (projection(origami.getVertices2d().get(origami.getPolygons().get(i).get(ii)))[0]) + xShift,
                            (short) (projection(origami.getVertices2d().get(origami.getPolygons().get(i).get(ii)))[1]) + yShift);
                }
    ...
    public void updateBuffer(Origami origami) {
        ...
        for (int i = 0; i < origami.getPolygonsSize(); i++) {
            ...
            if (isDrawable(i, origami, triangle)) {
                ...
                for (int ii = 0; ii < origami.getPolygons().get(i).size(); ii++) {
                    ...
                    path.addPoint((short) (new Camera(0, 0, 1d).projection(origami.getVertices2d().get(origami.getPolygons().get(i).get(ii)))[0]) + 200,
                            (short) (new Camera(0, 0, 1d).projection(origami.getVertices2d().get(origami.getPolygons().get(i).get(ii)))[1]) + 200);
                }
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (color != 0xFFFFFF) {
                ...
                try {
                    double x_1 = origami.getVertices2d().get(skeleton[color][1])[0] - origami.getVertices2d().get(skeleton[color][0])[0];
                    double x_2 = origami.getVertices2d().get(skeleton[color][1])[1] - origami.getVertices2d().get(skeleton[color][0])[1];
                    double y_1 = origami.getVertices2d().get(skeleton[color][2])[0] - origami.getVertices2d().get(skeleton[color][0])[0];
                    double y_2 = origami.getVertices2d().get(skeleton[color][2])[1] - origami.getVertices2d().get(skeleton[color][0])[1];
                    ...
                    double[] v3d1 = Geometry.vectorDiff(origami.getVertices().get(skeleton[color][1]), origami.getVertices().get(skeleton[color][0]));
                    double[] v3d2 = Geometry.vectorDiff(origami.getVertices().get(skeleton[color][0]), origami.getVertices().get(skeleton[color][2]));
                    spaceBuffer[i] = Geometry.vectorDiff(
                            Geometry.vectorDiff(Geometry.scalarMultiple(v3d1, lambda1), Geometry.scalarMultiple(v3d2, lambda2)),
